...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['indice']) && isset($_POST['novo_codigo'])) {
    $materias = obterMaterias();
    $indice = $_POST['indice'];
    $novo_codigo = $_POST['novo_codigo'];

    if (isset($materias[$indice])) {
        $materias[$indice]['codigo'] = $novo_codigo;
        salvarMaterias($materias);
        $mensagem = "Código da matéria alterado com sucesso!";
    } else {
...
<head>
    ...
    <title>Alterar Código da Matéria</title>
...
<section>
    <h1>Alterar Código da Matéria</h1>
    ...
    <?php if (count($materias) > 0): ?>
        <form method="post" action="">
            ...
            <label for="novo_codigo">Novo Código da Matéria:</label>
            <input type="text" id="novo_codigo" name="novo_codigo" placeholder="Digite o novo código" required>

            <input type="submit" value="Alterar Código">
        </form>
    <?php else: ?>
