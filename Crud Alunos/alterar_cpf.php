...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['indice']) && isset($_POST['novo_cpf'])) {
    $alunos = obterAlunos();
    $indice = $_POST['indice'];
    $novo_cpf = $_POST['novo_cpf'];

    if (isset($alunos[$indice])) {
        $alunos[$indice]['cpf'] = $novo_cpf;
        salvarAlunos($alunos);
        $mensagem = "CPF do aluno alterado com sucesso!";
    } else {
...
<head>
    ...
    <title>Alterar CPF do Aluno</title>
...
<section>
    ...
    <?php if (count($alunos) > 0): ?>
        <form method="post" action="">
            ...
            <label for="novo_cpf">Novo CPF do Aluno:</label>
            <input type="text" id="novo_cpf" name="novo_cpf" placeholder="Digite o novo CPF" required>

            <input type="submit" value="Alterar CPF">
        </form>
    <?php else: ?>
